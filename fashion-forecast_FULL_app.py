...
with tab1:
    ...
    # Top 10 Predicted Microtopics as a LIST
    st.subheader("Top 10 Predicted Microtopics (List)")

    top10_overview = (
        filtered_latest.sort_values("pred_next_engagement", ascending=False)
        .head(10)[["microtopic", "pred_next_engagement"]]
        .reset_index(drop=True)
    )

    for i, row in top10_overview.iterrows():
        st.write(
            f"**{i+1}. {row['microtopic']} — "
            f"{row['pred_next_engagement']:.2f} predicted engagement**"
        )
