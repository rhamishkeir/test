...
with tab3:
    ...
    if filtered_agg.empty:
        ...
        if predicted_next is not None and not hist_bi.empty:
            # Create 4 future weekly points right after the last historical week
            ...
            # For now we reuse the same predicted_next value for each of the 4 weeks
            forecast_rows = pd.DataFrame(
                {
                    "week_start": future_weeks,
                    "engagement": [predicted_next] * len(future_weeks),
