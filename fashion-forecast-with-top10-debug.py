...
art = load_artifacts()
st.write("DEBUG: artifacts loaded.")
...
st.write("DEBUG: core objects unpacked.")
...
if ts_agg is None or ts_agg.empty:
    st.error("ts_agg dataframe missing or empty in fashion_model.pkl")
    st.stop()
...
# filtered_ts  = apply_filters(ts_feat)  # not used anywhere

st.write(f"DEBUG: filtered_latest rows = {len(filtered_latest)}")
...
st.write("DEBUG: predictions computed.")
...
with tab1:
    ...
    MAX_ROWS_FOR_ITEM_AGG = 50000

    if len(filtered_latest) > MAX_ROWS_FOR_ITEM_AGG:
        st.info(
            f"Too many microtopics selected ({len(filtered_latest):,}). "
            "Use the Brand/Item filters to narrow down before computing item-level Top 10."
        )
    elif "item" in filtered_latest.columns and "pred_next_engagement" in filtered_latest.columns:
        try:
            ...
            if top10_items.empty:
                st.info("No item-level predictions available for this selection.")
            else:
                # Table view
                st.dataframe(
                    top10_items[
            ...
            st.error(f"Error while computing item-level Top 10: {e}")
    else:
