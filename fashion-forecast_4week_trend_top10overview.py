...
with tab1:
    ...
    # Table of top microtopics (filtered)
    ...
    # Top 10 next-week predictions as a bar chart
    st.subheader("Top 10 Predicted Microtopics for Next Week")

    top10_overview = filtered_latest.sort_values(
        "pred_next_engagement", ascending=False
    ).head(10)

    fig_top10_overview = px.bar(
        top10_overview,
        x="microtopic",
        y="pred_next_engagement",
        color="pred_next_engagement",
        title="Top 10 Predicted Microtopics (Next Week)",
    )
    st.plotly_chart(fig_top10_overview, use_container_width=True)
...
with tab3:
    ...
    if filtered_agg.empty:
        ...
        if predicted_next is not None and not hist_bi.empty:
            # Create 4 future weekly points using a simple trend calibrated to the model prediction
            # Sort history to ensure correct time order
            ...
            # If we have at least 2 historical points, fit a simple linear trend
            if len(hist_bi) >= 2:
                # Use an integer index as the time variable
                t = np.arange(len(hist_bi))
                y = hist_bi["engagement"].values

                # Fit a line: y = intercept + slope * t
                slope, intercept = np.polyfit(t, y, 1)

                # Future time steps for the next 4 weeks
                n_hist = len(hist_bi)
                t_future = np.arange(n_hist, n_hist + 4)
                y_future = intercept + slope * t_future

                # Calibrate so the first future point matches the model's next-week prediction
                offset = predicted_next - y_future[0]
                y_future = y_future + offset
            else:
                # Not enough history: fall back to a flat forecast from the model prediction
                y_future = np.array([predicted_next] * 4)

            future_weeks = [
                last_week + pd.Timedelta(weeks=i) for i in range(1, 5)
            ]
            ...
            forecast_rows = pd.DataFrame(
                {
                    ...
                    "engagement": y_future,
                    "type": ["Forecast"] * len(future_weeks),
                }
            )

            chart_df = pd.concat([hist_bi, forecast_rows], ignore_index=True)
